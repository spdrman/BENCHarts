...
export function formatCoord(n) {
  ...
  const rounded = (n < 0 ? -hundredths : hundredths) / 100;
  if (rounded === 0) return '0';
  if (Math.abs(rounded) < 1e21) return String(rounded);
  // Above 1e21 `String` switches to exponential, and `toFixed` is defined to
  // hand back exactly that same string rather than padding it out. Every double
  // that large is integer-valued, so BigInt prints the digits exactly. Nothing
  // should ever get here: R-VAL-3 caps a dimension at 16384 and R-VAL-4 refuses
  // a value past 1e300. This is the branch that keeps the function total anyway.
  return BigInt(Math.trunc(rounded)).toString();
}
